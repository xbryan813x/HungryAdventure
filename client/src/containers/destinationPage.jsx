...
class destinationPage extends Component {
  ...
  loadDestination = () => {
    if(this.props.current.destination.imageUrl.length > 0){
      return <div onClick={() => this.open(this.props.current.destination)}><img className="circleAdd" style={{ marginTop: '12vw',}} src={this.props.current.destination.imageUrl[0]}></img></div>
    }
  }

  loadHotel = () => {
    if(this.props.current.hotel.pictures.length > 0){
      return <div onClick={() => this.open(this.props.current.hotel)}><img className="circleAdd" style={{ marginTop: '23vw',}} src={this.props.current.hotel.pictures[0]}></img></div>
    }
  }

  loadEvents = () => {
    if(this.props.current.viatorEvents.length > 0){
        return <div onClick={() => this.open(this.props.current.viatorEvents)}><img className="circleAdd" style={{ marginTop: "34vw",}} src={this.props.current.viatorEvents[0].image}></img></div>
    }
  }

 loadFood = () => {
    if(this.props.current.yelpEvents.length > 0){
        return <div onClick={() => this.open(this.props.current.yelpEvents)}><img className="circleAdd" style={{ marginTop: "45vw",}} src={this.props.current.yelpEvents[0].image_url}></img></div>
    }
  }
